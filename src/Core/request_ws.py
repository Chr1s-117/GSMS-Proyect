...
class RequestWebSocketManager(WebSocketManager):
    ...
    async def _monitor_last_positions(self):
        ...
        Responsibilities:
        - Poll database every 0.5s for latest GPS data from all devices
        - Broadcast changes via GPS channel (add_gps)
        - Maintain cache to avoid duplicate broadcasts
        - Detect removed devices and clean cache
        """
        ...
        while self._monitor_last_positions_active:
            try:
                ...
                # Broadcast nuevas posiciones o cambios
                for device_id, gps_data in current_positions.items():
                    if device_id not in self._last_positions or \
                       self._last_positions[device_id] != gps_data:
                        add_gps(gps_data)
                        self._last_positions[device_id] = gps_data
                        print(f"[REQUEST-WS] Broadcasting GPS update for device {device_id}")
                ...
                for device_id in removed:
                    ...
                    print(f"[REQUEST-WS] Device {device_id} removed from cache")
    ...
    async def handle_message(self, ws: WebSocket, message: str):
        """
        Process incoming WebSocket messages and delegate to appropriate handlers.
        
        Supported actions:
        - ping: Health check
        - get_devices: List all active devices
        - get_history: Query GPS data within time range
        - get_timestamp_range: Get min/max timestamps for a device
        - get_last_positions: Get latest GPS for all devices (with subscription support)
        
        Args:
            ws: WebSocket connection
            message: JSON string with action, request_id, and params
        """
